...
//
// Plain WebGL1, no libraries: a fullscreen triangle carries the fragment
// shader below verbatim (see header comment inside it for the uniform packing).
...
const fragment = `
// "Dithering" — made with the 21st.dev Shader Builder
...
//   u_surface = vec4(1.82, 1.00, 0.00, 1.00)
//   u_finish = vec4(0.00, 0.00, 0.000, 0.04)
//   u_transform = vec4(4328.0, 0.00, 0.32, 1.0)
//   u_space = vec4(0.00, 0.00, pointer x, pointer y)
//   u_cursor = vec4(presence, 3.0, 0.65, 0.39)
...
export interface DitheringOptions {
  /** Four colours, low → high. Defaults to the packaged Paper Shaders preset. */
  colors?: [string, string, string, string];
  /** Seconds-per-second multiplier applied to elapsed time (speed 70/100). */
  timeSpeed?: number;
  scale?: number;
  intensity?: number;
  paramA?: number;
...
export function createDithering(container: HTMLElement, options: DitheringOptions = {}): () => void {
  const {
  ...
  // Fullscreen triangle: (-1,-1), (3,-1), (-1,3) — clipped to the viewport.
  ...
  const paletteColors = colors.map(hexToRgb);
  for (let i = 0; i < 8; i++) {
    const [r, g, b] = paletteColors[Math.min(i, paletteColors.length - 1)];
    colorArray[i * 3] = r;
    colorArray[i * 3 + 1] = g;
    colorArray[i * 3 + 2] = b;
  }
  gl.uniform3fv(uColors, colorArray);
  gl.uniform4f(uShape, scale, intensity, paramA, warp);
  gl.uniform4f(uSurface, detail, contrast, brightness, saturation);
  gl.uniform4f(uFinish, hue, vignette, blur, grain);
  gl.uniform4f(uTransform, seed, rotate, drift, oklab);

  let width = 1;
  let height = 1;
  // Capped lower than the display's true DPR: this is a full-viewport-height
  // background running an fbm shader every frame, so pixel count matters a
  // lot more than crispness here.
  ...
  // Only true while some part of the container is on-screen — skips the RAF
  // loop entirely for a hero/footer background scrolled out of view instead
  // of burning GPU time on pixels nobody sees (and competing with scroll).
  let isInViewport = false;
  const t0 = performance.now();

  function render() {
    gl.uniform4f(uScene, width, height, ((performance.now() - t0) * 0.001) * timeSpeed, 4.0);
  ...
  const tryStart = () => {
    ...
    if (isPageVisible && isInViewport && raf === 0) {
      raf = requestAnimationFrame(loop);
    }
  };
  const tryStop = () => {
  ...
  const io = new IntersectionObserver(
    ([entry]) => {
      ...
      if (isInViewport) {
        // Render one frame immediately so it's not showing a stale/blank
        // canvas for the gap before the next RAF tick.
        render();
        tryStart();
      } else {
        tryStop();
